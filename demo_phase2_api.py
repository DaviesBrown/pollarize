...
class PollarizeAPIDemo:
    ...
    def authenticate(self, username: str = "alice_tech", password: str = "demo123"):
        ...
        if response.status_code == 200:
            ...
            if data.get('success'):
                ...
                print(f"👤 User: {self.user_data['username']} ({self.user_data['email']})")
            else:
    ...
    def demo_categories(self):
        ...
        response = requests.get(f"{self.api_base}/categories/", headers=self.get_headers())
    ...
    def demo_user_profiles(self):
        ...
        # Get user profiles
        response = requests.get(f"{self.api_base}/profiles/", headers=self.get_headers())
    ...
    def demo_enhanced_polls(self):
        ...
        response = requests.get(f"{self.api_base}/polls/", headers=self.get_headers())
        ...
        if polls_data.get('success') and polls_data['data']['results']:
            ...
            response = requests.get(f"{self.api_base}/polls/{poll_id}/", headers=self.get_headers())
            self.print_response(response, "Poll Detail (Full Serializer with is_bookmarked)")
    ...
    def demo_bookmarks(self):
        ...
        if polls_data.get('success') and polls_data['data']['results']:
            ...
            # List user bookmarks
            response = requests.get(f"{self.api_base}/bookmarks/", headers=self.get_headers())
            ...
            self.print_response(response, "Try to Bookmark Same Poll Again (Should Fail)")
    ...
    def demo_poll_sharing(self):
        ...
        if polls_data.get('success') and polls_data['data']['results']:
            ...
            if share_data.get('success'):
                ...
                response = requests.get(f"{self.api_base}/shares/{referral_code}/track/")
    ...
    def demo_search_and_filtering(self):
        ...
        self.print_response(response, "Order Polls by Creation Date (Newest First)")
    ...
    def demo_poll_detail_with_bookmark_status(self):
        ...
        if polls_data.get('success') and polls_data['data']['results']:
            ...
            response = requests.get(f"{self.api_base}/polls/{poll_id}/", headers=self.get_headers())
            self.print_response(response, "Poll Detail Showing Bookmark Status")
    ...
    def run_full_demo(self):
        ...
        if not self.access_token:
            print("❌ Could not authenticate. Make sure the server is running and sample data is loaded.")
            return
