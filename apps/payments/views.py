...
class PaymentViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def verify(self, request, pk=None):
        ...
        try:
            ...
            service = PaymentService()
            success, message = service.verify_payment(payment.provider_reference)
...
class RefundViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        return Refund.objects.filter(
            Q(requested_by=self.request.user) | Q(payment__user=self.request.user)
    ...
    def create_request(self, request):
        ...
        refund, error = service.create_refund(
            ...
            reason_description=serializer.validated_data.get('reason_description', ''),
...
class ReferralRewardViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def summary(self, request):
        ...
        total_rewards = rewards.aggregate(total=Sum('amount'))['total'] or Decimal('0.00')
        pending_rewards = rewards.filter(is_paid=False).aggregate(total=Sum('amount'))['total'] or Decimal('0.00')
